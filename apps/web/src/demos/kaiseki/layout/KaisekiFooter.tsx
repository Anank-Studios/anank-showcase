 ...
 *
 * O AVISO DE DEMONSTRAÇÃO é obrigatório e não pode depender de hover, acordeão
 * ou rolagem infinita: quem chega por link direto precisa conseguir ler que a
 * casa é fictícia e que nenhum pedido é cobrado. Numa demo que MOSTRA botão de
 * pagamento, isso vale dobrado.
 */
export function KaisekiFooter({ demo }: { demo: Demo }) {
  return (
    <footer className="border-t border-line bg-[color:var(--brand-surface)]">
      <div className="mx-auto max-w-[1400px] px-5 py-16 md:px-10 md:py-20 lg:px-14">
        <div className="grid gap-12 md:grid-cols-[1.4fr_1fr_1fr]">
          <div>
            <p className="font-display text-[clamp(2rem,5vw,3.25rem)] leading-[1.05]">Kaiseki</p>
            ...
            <ul className="mt-4 space-y-2">
              {demo.hours.map((h) => (
                <li key={h.day} className="flex justify-between gap-4 text-[14px]">
                  ...
                  <span className="shrink-0 text-right">{h.open}</span>
                </li>
              ))}
            </ul>

            <nav className="mt-7 flex flex-col gap-1.5" aria-label="Navegação do rodapé">
              <IntentLink href="/demo/kaiseki/cardapio" className="text-[14px] hover:text-accent">
                Cardápio
              </IntentLink>
              <IntentLink href="/demo/kaiseki/visite" className="text-[14px] hover:text-accent">
                A casa
              </IntentLink>
              <IntentLink href="/demo/kaiseki/sacola" className="text-[14px] hover:text-accent">
                Sacola
              </IntentLink>
            </nav>
          </div>
        ...
        <div className="mt-14 border-t border-line pt-7">
          <p className="max-w-[80ch] text-[12px] leading-relaxed text-muted">
            ...
            Estabelecimento, endereço, CNPJ, telefone e depoimentos são fictícios. O fluxo de pedido
            funciona de ponta a ponta, mas{' '}
            <strong className="font-semibold text-[color:var(--brand-ink)]">
              nenhum pedido é enviado a uma cozinha, armazenado ou cobrado
            </strong>
            .
          </p>
        </div>
